...
```
├── Makefile        # 入口 (up / argocd-info / argocd-render / status / marker-* / eso-up / down)
├── docs/           # 予習用ドキュメント (事前知識 / ArgoCD / CRD / ESO)
├── kind/           # 各クラスタの kind 設定 (Gitea / demo-app の NodePort をホストへ公開)
├── hub/            # hub クラスタに置くもの
